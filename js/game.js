...
function preload() {
    ...
    // Fireball frames
    for (let i = 1; i <= 5; i++) {
        this.load.image(`fireball${i}`, `assets/fireball${i}.png`);
    }
...
function create() {
    ...
    for (let i = 0; i < player.health; i++) {
        ...
        heart.fillStyle(0xff0000); // Red
        heart.fillRect(0, 0, heartSize, heartSize);
        heart.x = padding + i * (heartSize + padding);
        heart.y = padding;
        heart.setScrollFactor(0);
        heart.setDepth(20);
        player.hearts.push(heart);
    }
    ...
    const bossScale = 2.4;
    boss = this.physics.add.sprite(castle.x + castle.displayWidth + 500, 0, "demon_walk_1");

    // Use setScale for consistent sprite size across all animations.
    ...
    const healthBarY = boss.y - boss.displayHeight / 2 - 20;

    boss.healthBarBg = this.add.graphics();
    boss.healthBarBg.fillStyle(0xff0000); // Red background
    boss.healthBarBg.fillRect(0, 0, healthBarWidth, healthBarHeight);
    boss.healthBarBg.x = healthBarX;
    boss.healthBarBg.y = healthBarY;
    boss.healthBarBg.setDepth(10);

    boss.healthBar = this.add.graphics();
    boss.healthBar.fillStyle(0x00ff00); // Green foreground
    boss.healthBar.fillRect(0, 0, healthBarWidth, healthBarHeight);
    boss.healthBar.x = healthBarX;
    boss.healthBar.y = healthBarY;
    boss.healthBar.setDepth(10);
    ...
    // Fireball hits boss
    this.physics.add.overlap(this.fireballs, boss, hitBossWithFireball, null, this);
...
function update() {
    ...
    // Check fireball travel distance
    this.fireballs.getChildren().forEach(fireball => {
        if (fireball.active) {
            const travelDistance = Math.abs(fireball.x - fireball.startX);
    ...
    if (boss && boss.active) {
        ...
        const healthPercentage = Math.max(0, boss.health / 100);
        boss.healthBar.clear();
        boss.healthBar.fillStyle(0x00ff00);
        boss.healthBar.fillRect(0, 0, healthBarWidth * healthPercentage, healthBarHeight);

        boss.healthBar.setVisible(boss.visible);
        boss.healthBarBg.setVisible(boss.visible);

        // Boss AI Logic
        if (boss.state === 'idle' && !dialogueActive) {
            ...
            if (distance < 120) { // close enough to attack
                ...
                boss.once("animationcomplete-demon_attack", () => {
                    // Check if player is still in range when attack animation finishes
                    const playerStillInRange = Phaser.Math.Distance.Between(player.x, player.y, boss.x, boss.y) < 150;

                    if (boss.active && playerStillInRange) {
                        damagePlayer.call(this, player);
                    }

                    // Reset boss state after attack
...
function shootFireball() {
    ...
    const offset = player.flipX ? -30 : 30; // adjust spawn side based on direction
    const fireball = this.fireballs.create(player.x + offset, player.y, "fireball1");

    fireball.startX = fireball.x; // Store starting position

    fireball.setScale(1.2);
    fireball.anims.play("fireball_anim", true);

    // Flip the fireball sprite to match the player's direction
    fireball.setFlipX(player.flipX);

    // Set velocity based on direction
    const speed = 300;
    fireball.setVelocityX(player.flipX ? -speed : speed);
}
...
function playerBossCollision(player, boss) {
    ...
    if (player.body.velocity.y > 0 && player.y < boss.y - (boss.displayHeight * 0.25)) {
        ...
        if (boss.health <= 0) {
            ...
            boss.once('animationcomplete-demon_death', () => {
            ...
            boss.once('animationcomplete-demon_take_hit', () => {
                if (boss.active && boss.state === 'taking_damage') {
                    // Re-apply the constant physics body.
                    const bodyWidth = boss.width * 0.4;
                    const bodyHeight = boss.height * 0.9;
...
function damagePlayer(player) {
    ...
    if (player.hearts.length > 0) {
        const heart = player.hearts.pop();
        heart.destroy();
    }

    if (player.health <= 0) {
        resetPlayerHealth.call(this, player);
    } else {
...
function resetPlayerHealth(player) {
    ...
    player.hearts.forEach(heart => heart.destroy());
    player.hearts = [];
...
function hitBossWithFireball(boss, fireball) {
    // Destroy the fireball on impact.
    fireball.destroy();

    // Prevent new hits while boss is already reacting (taking damage or dying).
    if (boss.state === 'taking_damage' || boss.state === 'dead') {
        return;
    }
    ...
    if (boss.health <= 0) {
        // --- DEATH SEQUENCE ---
        boss.state = 'dead';
        boss.setVelocityX(0); // Stop movement.
        boss.play('demon_death', true); // Play death animation.

        // On animation complete, destroy the boss object.
        boss.once('animationcomplete-demon_death', () => {
            if(boss.healthBar) boss.healthBar.destroy();
            if(boss.healthBarBg) boss.healthBarBg.destroy();
            boss.destroy();
        ...
        // --- TAKE HIT SEQUENCE ---
        boss.state = 'taking_damage';
        boss.play('demon_take_hit', true); // Play take-hit animation.

        // After the hit animation finishes, reset the physics and state.
        boss.once('animationcomplete-demon_take_hit', () => {
            // Check if the boss is still active and in the correct state before proceeding.
            if (boss.active && boss.state === 'taking_damage') {
                // Re-apply the constant physics body to ensure it hasn't changed.
                const bodyWidth = boss.width * 0.4;
                const bodyHeight = boss.height * 0.9;
                boss.body.setSize(bodyWidth, bodyHeight);
                boss.body.setOffset(boss.width * 0.3, boss.height * 0.1);
...
function createMobileControls() {
    ...
    // Jump Button
    const jumpBtnX = this.scale.width - buttonSize - padding;
    const jumpBtn = this.add.rectangle(jumpBtnX, y, buttonSize, buttonSize, 0xff6666)
        .setOrigin(0).setScrollFactor(0).setInteractive().setDepth(20);
    this.add.text(jumpBtnX + buttonSize * 0.25, y + buttonSize * 0.25, "↑", {
        fontSize: `${buttonSize * 0.5}px`,
        color: "#fff",
    }).setScrollFactor(0).setDepth(21);
    ...
    // Fireball Button
    const fireBtnX = this.scale.width - (buttonSize * 2) - (padding * 2);
    const fireBtn = this.add.rectangle(fireBtnX, y, buttonSize, buttonSize, 0xffa500) // Orange color
        .setOrigin(0).setScrollFactor(0).setInteractive().setDepth(20);

    this.add.text(fireBtnX + buttonSize * 0.35, y + buttonSize * 0.25, "F", {
        fontSize: `${buttonSize * 0.5}px`,
        color: "#fff",
    }).setScrollFactor(0).setDepth(21);
